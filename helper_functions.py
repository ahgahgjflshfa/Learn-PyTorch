...
def train_step(model: torch.nn.Module,
    ...
    Returns:
        tuple[float, float]: A tuple containing train loss and train accuracy. (train_loss, train_accuracy)
    """
...
def test_step(model: torch.nn.Module,
    ...
    Returns:
        tuple[float, float]: A tuple containing test loss and test accuracy. (test_loss, test_accuracy)
    """
...
def train(model: torch.nn.Module,
    ...
    """Train function to train and test `model`

    Args:
        model: The model to train.
        train_dataloader: Data loader for train datas.
        test_dataloader: Data loader for test datas.
        loss_fn: Loss function to evaluate the model's loss.
        optimizer: The optimizer to use.
        acc_fn: Accuracy function to evaluate the accuracy.
        device: Which device to train on.

    Returns:
        dict[str, list[float]]: A dictionary containing all train loss, train accuracy, test loss and test accuracy
...
def eval_model(model: torch.nn.Module,
               ...
               acc_fn,
               device: str = 'cpu'):
    """Evaluates `model`'s loss and accuracy
    ...
    Returns:
        dictionary: A dictionary containing the loss and accuracy of model predicting on *data_loader*
    """
    loss, acc = 0, 0
    model.eval()
    with torch.inference_mode():
...
def plot_transformed_images(image_paths: list, transform: torchvision.transforms, n=3, seed: int=None):
    """Selects random images from a path of images and loads/transforms them then plots the original vs the transformed
    version

    Args:
        image_paths (list): A list of image paths to select from.
    ...
    for image_path in random_image_paths:
        with Image.open(image_path) as img:
            ...
            transformed_image = transform(img).permute(1, 2, 0) # (3, 64, 64) -> (64, 64, 3)
